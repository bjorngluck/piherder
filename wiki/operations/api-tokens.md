...
Off by default. Server-side n8n/HA/curl do **not** need CORS. Set `CORS_ORIGINS` only for browser apps on other origins (exact origins; never `*`). A first-class **HACS integration** (HA dashboard of the fleet) is **v1.6 Must** ([PLAN_v1.6.0.md](https://github.com/bjorngluck/piherder/blob/v1.6.0-dev/docs/PLAN_v1.6.0.md)). Until it ships, use `rest` sensors as in [API.md](https://github.com/bjorngluck/piherder/blob/main/docs/API.md).
