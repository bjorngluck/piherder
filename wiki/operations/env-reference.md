...
| `PIHERDER_SERVICE_MIGRATE` | **false** (default) — **master enable** for Docker **Move to another host…** (stop → copy → dest up → DNS/NPM). Host **lock / unlock** is always available (operator+). Optional leftover: leave stopped, `compose down`, or remove source project + named volumes (extra ack). Leave off until you want operators using copy. Demo never copies. |
