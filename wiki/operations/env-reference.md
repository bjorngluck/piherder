...
| `PIHERDER_SERVICE_MIGRATE` | **false** (default) — **master enable** for Docker **Move to another host…**. Host **lock / unlock** is always available (operator+). Leave off until the migrate pipeline is complete enough. Demo never copies. |
