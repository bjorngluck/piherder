...
| `PIHERDER_CSP` | **true** (default) — send Content-Security-Policy. Scripts are **self-hosted** (compiled Tailwind, no Play CDN, **no `unsafe-eval`**). `connect-src` is `'self'` plus `PIHERDER_PUBLIC_URL` / its `wss:` — **no** wildcard `ws:`/`wss:`. Inline scripts get a per-request **nonce** (`script-src` drops `'unsafe-inline'`). `onclick` stays via `script-src-attr 'unsafe-inline'`. Styles stay `'unsafe-inline'`. |
| `PIHERDER_CSP_REPORT_ONLY` | **false** (default) — if true, send Report-Only CSP instead of enforcing. Public demo compose sets this **true** |
| `PIHERDER_CSP_ENFORCE` | **false** (default) — set **true** on a demo host only when you want that public site to enforce the nonce policy. Until then, demo mode stays Report-Only |
| `PIHERDER_SSH_CONSOLE` | **false** (default) — **master enable** for web SSH (operator+ / 2FA; in-app only). Not a Settings checkbox. Per-host **Console mux** is Edit → Features (`console_mux_enabled`), not an env flag. |
...
| `PIHERDER_SERVICE_MIGRATE` | **false** (default) — **master enable** for Docker **Move to another host…** (stop → copy → dest up → DNS/NPM). Job runs on **celery-worker** (v1.5); recycle **web** is safe. Host **lock / unlock** is always available (operator+). Optional leftover: leave stopped, `compose down`, or remove source project + named volumes (extra ack). Optional **Adopt into fabric** on preflight (default off). Leave off until you want operators using copy. Demo never copies. After toggling, recreate **web** (and **celery-worker** if the image changed). [Move a service](../docker/service-migration.md). |
...
- [v1.4.0 QA](https://github.com/bjorngluck/piherder/blob/main/docs/QA_v1.4.0.md) (maintainer `docs/` — not this wiki)  
