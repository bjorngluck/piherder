...
**Fleet mutate** means starting backups, OS/container patch and checks, Docker compose actions, template deploy, **Move a service** (when `PIHERDER_SERVICE_MIGRATE` is on), integration binds, cert deploy, bulk servers actions, **Host Files** (when `PIHERDER_HOST_FILES` is on), etc. Viewers never start migrate (403). On a real herder, viewers never see the Files button (403). On the **public demo**, viewers may **browse** the canned Files tree; writes are refused. Privileged Files follows the same “who may elevate” knob as the console. Opening `.env` / PEMs in Files needs the same 2FA grant as privileged Files (operators can do that on fleet without elevate).
