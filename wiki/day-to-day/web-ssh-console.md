...
**Not GNU `screen` / `tmux` by default.** Each console is a direct SSH PTY. Soft resume parks on the **herder**, not on the host — so recreate **web** (or a herder crash) ends parked shells. You can still run `screen`/`tmux` yourself if installed. A later **opt-in per host** mux (prefer tmux, then screen, else plain PTY) is parked for a future train — not this image. [PLAN_v1.5.0 W-mux](https://github.com/bjorngluck/piherder/blob/v1.5.0-dev/docs/PLAN_v1.5.0.md).
