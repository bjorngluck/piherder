...
- Ship an HA custom component **in this image**. Path 2 (HA **observes** the fleet) is a **separate** HACS repo (plugin **0.2.2**) — operator: [Home Assistant → PiHerder](../integrations/home-assistant.md). Device page **Visit** is the host; fleet totals and section chips are the Lovelace card. YAML `rest` remains possible.
