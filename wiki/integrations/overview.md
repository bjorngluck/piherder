...
!!! tip "Home Assistant"
    ...
    **Path 2:** HA **observes** the PiHerder fleet via HACS — [Home Assistant → PiHerder](home-assistant.md). Poll reads snapshots only. Plugin **0.3.0**: Visit on the host device; fleet card; host, updates, and resources cards. A `read` token shows sensors only. `jobs` and `edit` show the confirms and feature toggles.
