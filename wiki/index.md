...
| **Next** | **v1.7.0** on `v1.7.0-dev`. [Agents (MCP)](operations/mcp.md) **0.1.0** (own repo, read and write of today’s API token). On the branch: remaining exclusive jobs on Celery, a Settings wait for a host that is down, and [Appearance](getting-started/appearance.md) (instance name, one accent, hide Catalog in the nav). Operator walks are still open. Coverage 75 → 80 is not started. [Home Assistant](integrations/home-assistant.md) **HA-cards** (more cards and confirm writes) is planned and is not in plugin **0.2.4**. Technical: [PLAN_v1.7.0.md](https://github.com/bjorngluck/piherder/blob/v1.7.0-dev/docs/PLAN_v1.7.0.md). Production stays **v1.6.0**. |
