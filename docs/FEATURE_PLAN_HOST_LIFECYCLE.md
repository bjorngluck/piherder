...
| # | Decision |
|---|----------|
| 1 | **v0.6.0 RC2** — P1 Docker bulk **shipped**; **P2 wizard product-done in v0.7.0** — [PLAN_v0.7.0.md](PLAN_v0.7.0.md) |
| 2 | Ship order = **risk/value:** Docker bulk (0.6) → wizard (0.7) → stats/commands (0.8 capacity) → bootstrap/DNS → web SSH last |
...
- **Host-side `screen` / `tmux` for web console** — **v1.5 Discover W-mux written** (2026-09-13). Opt-in per host; tmux then screen else PTY; herder park stays. Job **Mux-1** parked [PLAN_v1.6.0.md](PLAN_v1.6.0.md). Today: plain `invoke_shell` + herder soft-park only.  
