...
**Status:** Docker Hub **live** — [bjorngluck/piherder](https://hub.docker.com/r/bjorngluck/piherder) (public). Multi-arch **linux/amd64 + linux/arm64** published for **v0.9.0**.  
...
| Tag | Meaning |
|-----|---------|
| `0.9.0` | Immutable release (match git tag `v0.9.0`) |
| `0.9` | Optional rolling minor |
| `latest` | Current stable RC/release |
| `0.9.0-dev` / `main` | Optional CI/dev only — avoid as default for operators |
...
export VERSION=0.9.0   # match release
