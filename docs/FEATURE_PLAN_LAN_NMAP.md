...
**Status:** **Approved** (2026-07-19) — **N0–N10 product complete** on main: worker, devices, schedules, presets, soft embed, **kind heuristics + override**, **map identity** (name / gateway role), **known/new lifecycle**, **MAC identity / DHCP**, **Hosts map** (radar toggle, compact 1:1 fit, one-line chrome), **centered edit modal**. **v0.9 chrome:** Devices **List | Map** (merged former Network tab); server LAN chip `return=server:{id}`; Overview shortcuts trimmed; Schedules/Runs **one dense list** at all widths (no dual table/cards) — [PLAN_v0.9.0.md](PLAN_v0.9.0.md) · operator wiki [lan-discovery.md](../wiki/integrations/lan-discovery.md). Screenshots may lag chrome.
...
| 2026-07-26 | **Scan now:** replace browser `confirm` with in-app modal; wait “Queueing scan” only after accepted queue (never on Cancel). Path map NPM hub multi-path focus (logical `data-node-id`). Discovery hygiene (last seen / purge / hide) → **v1.1** default ([PLAN_v1.1.0.md](PLAN_v1.1.0.md) S). |
| 2026-07-29 | **v1.1 elevate production** on `v1.1.0-dev` — discovery hygiene **S** is stream **B** Must ([PLAN_v1.1.0.md](PLAN_v1.1.0.md)). |
