...
| **Host Files** | Default off (`PIHERDER_HOST_FILES=false`). Dest-card on every SSH host (including HAOS). Fleet jail = docker_base or home; privileged jail = `/` minus virtual FS. Operator+; API scope `files` (fleet). Upload cap 512 MiB (env `PIHERDER_HOST_FILES_MAX_BYTES`, ceiling 2 GiB). UI also edits UTF-8 text (512 KiB), zips a selection, unzips with zip-slip refusal, recursive delete, chmod/chown (privileged + `sudo -n`). Wiki: [host-files](../wiki/day-to-day/host-files.md) |
...
Active ship plan: [PLAN_v1.3.0.md](PLAN_v1.3.0.md) on `v1.3.0-dev`. Current production: [RELEASE_v1.2.0.md](RELEASE_v1.2.0.md) · [QA_v1.2.0.md](QA_v1.2.0.md). Security model: [SECURITY.md](../SECURITY.md).
