...
| **Service migrate** | Default off (`PIHERDER_SERVICE_MIGRATE=false`) for **Move to another host…**. Project **Lock to this host…** is always available (operator+). HAOS is never a migrate source/dest. Preflight only until copy ships. Wiki: [Move a service](../wiki/docker/service-migration.md) |
