...
| **Service migrate** | Default off (`PIHERDER_SERVICE_MIGRATE=false`) for **Move to another host…**. Project **Lock to this host…** is always available (operator+). HAOS is never a migrate source/dest. Flag on: stop → copy → dest up → DNS/NPM → TLS/Kuma validate → rebind. Optional source `compose down`, or **remove** source project + named volumes (extra ack; dest never wiped). Wiki: [Move a service](../wiki/docker/service-migration.md) |
