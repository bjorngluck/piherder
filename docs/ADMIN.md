...
| **Service migrate** | Default off (`PIHERDER_SERVICE_MIGRATE=false`) for **Move to another host…**. Project **Lock to this host…** is always available (operator+). HAOS is never a migrate source/dest. Flag on: stop → copy → dest `up -d` → direct CNAME + both Pi-holes `restartdns`, or NPM `forward_host` PUT. Wiki: [Move a service](../wiki/docker/service-migration.md) |
