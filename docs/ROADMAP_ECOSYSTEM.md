...
| **v1.5.0** | **Job runtime** — Move on **Celery worker** (**M-worker**, landed) + **N3a** Reports pin/hide/reorder (landed) + unit **≥ 65%**. Discover: rollback, CSP nonces, branding, migrate default, W-mux, HAOS path-2. **AC-fg out.** | Post-1.4 minor | **Active** on `v1.5.0-dev` — [PLAN_v1.5.0.md](PLAN_v1.5.0.md) · [QA_v1.5.0.md](QA_v1.5.0.md). Package stays `1.4.0` until freeze |
...
**Progress (2026-09-07):** **M-worker landed** (Celery `service_migrate`, dual-host backup mutex). **N3a landed** (cookie `ph_reports_layout`; operator signed). **Q** unit **~65%**; CI fail-under **65**. Leftover: recycle web/worker mid-Move live QA, Discover write-ups, freeze (**M-flag**, tag, Hub).
...
| **Insights / custom dashboards (N)** | **N2 shipped v1.3** — `/reports` history. **N3a landed on `v1.5.0-dev`** — pin/hide/reorder existing cards per browser cookie. **N3c** widget picker stays Out. Not Grafana-in-herder. [PLAN_v1.5.0.md](PLAN_v1.5.0.md). |
