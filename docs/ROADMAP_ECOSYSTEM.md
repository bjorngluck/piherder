...
| **v1.6.0** | **Candidate** — HAOS path 2 **plugin** (HA → PiHerder custom integration; discover in 1.5). Parked: **AC-fg** fine-grained roles · unit coverage step toward **1.x 80%** · **archive 0.x PLAN/RELEASE**. | Post-1.5 minor | Under consideration — [PLAN_v1.6.0.md](PLAN_v1.6.0.md) · [FEATURE_PLAN_HOME_ASSISTANT.md](FEATURE_PLAN_HOME_ASSISTANT.md) §7 |
...
**Progress (2026-09-08):** **M-worker landed** (Celery `service_migrate`, dual-host backup mutex). **N3a landed** (cookie `ph_reports_layout`; operator signed). **N3b landed** (Move jobs card). **Q** unit **~70.6%**; CI fail-under **70**. Leftover: recycle web/worker mid-Move live QA, Discover write-ups, freeze (**M-flag**, tag, Hub).
