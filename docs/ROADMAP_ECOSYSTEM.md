...
| **v1.5.0** | **Job runtime** — Move on **Celery worker** (**M-worker**, landed) + **N3a** Reports pin/hide/reorder + **N3b** Move jobs card (landed) + unit **≥ 70%**. Discover: rollback, CSP nonces, branding, migrate default, W-mux, HAOS path-2. **AC-fg out.** | Post-1.4 minor | **Active** on `v1.5.0-dev` — [PLAN_v1.5.0.md](PLAN_v1.5.0.md) · [QA_v1.5.0.md](QA_v1.5.0.md). Package stays `1.4.0` until freeze |
| **v1.6.0** | **Candidate** — HAOS path 2 **plugin** (HA → PiHerder custom integration; discover in 1.5). Parked: **AC-fg** · unit coverage step toward **1.x 80%** · **archive 0.x PLAN/RELEASE** · **CSP-n Slice 1** (script nonces + `script-src-attr`). | Post-1.5 minor | Under consideration — [PLAN_v1.6.0.md](PLAN_v1.6.0.md) · [FEATURE_PLAN_HOME_ASSISTANT.md](FEATURE_PLAN_HOME_ASSISTANT.md) §7 |
...
| **Web console host mux (`screen`/`tmux`)** | **v1.5 Discover (W-mux)** — optional host-side multiplexer for durable reattach beyond herder soft-park. Low priority; M-worker is green — still Discover, do not start unless pulled. [PLAN_v1.5.0.md](PLAN_v1.5.0.md). |
...
| Area | Direction |
|------|-----------|
| Home Assistant | **0.9 path 1 done (HAOS/S2 over SSH)** — [FEATURE_PLAN_HOME_ASSISTANT.md](FEATURE_PLAN_HOME_ASSISTANT.md): auto-mark, System Info, OS check/apply via `ha` CLI; deps SSH add-on + rsync. **v1.5 Discover / v1.6 ship:** path 2 custom integration (HA → PiHerder). API tokens already work ([API.md](API.md)). |
