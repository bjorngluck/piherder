...
| **v1.6.0** | **HACS on HA + console mux** — Slice 1 fleet+host devices (Must) · Mux-1 per-host tmux/screen (Must) · unit **≥ 75%**. Should: Slice 1b snapshot entities · 0.x archive · CSP-n Slice 1 · Undo-1. | Post-1.5 minor | **Active** on `v1.6.0-dev` — [PLAN_v1.6.0.md](PLAN_v1.6.0.md) · [FEATURE_PLAN_HOME_ASSISTANT.md](FEATURE_PLAN_HOME_ASSISTANT.md) §7 |
...
| **Web console host mux (`screen`/`tmux`)** | **v1.6 Must (Mux-1)** — opt-in per host; tmux then screen else PTY; herder park stays. Mux-2 (reattach after web recycle) Discover. [PLAN_v1.6.0.md](PLAN_v1.6.0.md). |
