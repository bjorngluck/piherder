...
**v1.7 (opened, not shipped):** **MCP-1** (Must, first) is a separate stdio process (`bjorngluck/piherder-mcp`, once that repo exists). It is a client of the routes above (`read`, and `jobs` / `edit` / `files` when the token has them). It is not in this image and it does not add `/api/v1` routes. **Jr-1** (Must, second) moves the remaining exclusive job types onto Celery and also adds no routes. [PLAN_v1.7.0.md](PLAN_v1.7.0.md).
