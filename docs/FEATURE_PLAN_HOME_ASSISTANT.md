...
**Status:** **v0.9.0 path 1 shipped (S2 HAOS over SSH)**. Path 2 Slice 1 and 1b shipped in **v1.6.0** (plugin **0.2.4**, read-only). **v1.7 Should HA-cards** is not started ([PLAN_v1.7.0.md](PLAN_v1.7.0.md)). The job-finished bus event stays Discover. Slice 3 stays out.  
...
| 6 | **Never** from HA: Move, compose write, Files, console, decrypt keys, container start/stop. **`service_migrate` stays off** `POST /api/v1/…/jobs`. OS apply is not a Slice 1 action. **HA-cards** may enqueue the existing `os_patch` and `container_patch` job types after a confirm. |
...
| **HA-cards** | Host card, updates card, and confirm writes for today’s token job types and the backup / OS-patch / Docker feature flags. No new herder routes. Plugin stays a separate repo | **v1.7 Should.** Not started. Plugin **0.2.4** stays read-only. May slip |
