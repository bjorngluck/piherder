...
| **v1.0.x** | Prior production; prefer upgrade to **v1.4.x** ([RELEASE_v1.0.0.md](docs/RELEASE_v1.0.0.md)) |
...
| Agent MCP (v1.7, separate repo) | stdio process on the operator machine. Holds `PIHERDER_URL` and the Bearer token there. The herder does not open an MCP port. Writes stay inside `jobs`, `edit`, and `files`. [wiki/operations/mcp.md](wiki/operations/mcp.md) |
...
| **Content-Security-Policy (v1.2+)** | Default **on** (`PIHERDER_CSP=true`): `default-src 'self'`, `object-src 'none'`, `frame-ancestors 'self'` (same-origin console modal only), `frame-src 'self'`, form-action self, **connect-src `'self'` + public origin / its `wss:` only** (no wildcard `ws:`/`wss:`). **No `unsafe-eval`** (Tailwind is compiled CSS). **v1.6 CSP-n Slice 1:** each response gets a script nonce. `script-src` is `'self' 'nonce-…'` (no `'unsafe-inline'`). `script-src-attr 'unsafe-inline'` keeps `onclick`. `style-src` stays `'unsafe-inline'`. `/docs` and `/redoc` still allow `'unsafe-inline'`. Home installs **enforce**. Demo mode sends **Report-Only** unless `PIHERDER_CSP_ENFORCE=true`. Also: `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, `Permissions-Policy` (incl. `publickey-credentials-get=(self)` for passkeys in console iframe). |
...
- **CSP (v1.2+, nonces v1.6):** leave `PIHERDER_CSP=true` in production. Home installs enforce `script-src 'nonce-…'` plus `script-src-attr 'unsafe-inline'`. Use `PIHERDER_CSP_REPORT_ONLY=true` only while validating a tighter policy. Demo mode stays Report-Only unless `PIHERDER_CSP_ENFORCE=true`. Console assets (xterm) are vendored under `/static/vendor/xterm/` so they need no CDN allowlist. When Turnstile is configured, CSP allows `https://challenges.cloudflare.com` for script/frame/connect.  
...
- **Where SSH jobs run (v1.7):** backup, Move, undo, OS/container patch, update checks, stack and template jobs, and `host_reboot` execute on **celery-worker** (default queue). That container holds the same decrypted host keys those jobs need. Recycling **web** does not stop them. Recycling the worker fails a **running** mutate (it is not resumed). `retention`, the herder’s own backup, host-facts snapshots, the web console, and Host Files stay in the **web** process. nmap runs only on `celery-worker-nmap` (`PIHERDER_NMAP_WORKER=1`). `host_reboot` needs scope `jobs` plus `feature:os` and the host OS-patch flag. It is **409** while an OS patch, container patch, or backup is active on that host, and those three are **409** while a reboot is active. The MCP adapter does not expose `host_reboot`, SSH, the console, or Move.  
