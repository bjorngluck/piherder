...
- **Host Files** (v1.3, default **off**, `PIHERDER_HOST_FILES=false`): jailed SFTP (list / get / put / mkdir / rename / delete empty). Operator+; viewer 403; demo never. Default identity **fleet** (docker_base or home, never `/`). Optional **privileged** Connect as… (same elevate RBAC + 2FA grant as console; jail `/` minus `/proc` `/sys` `/dev` `/run`). API scope `files` is fleet-only. Uploads stream (default 512 MiB). Audit `host_file_*` records path + bytes + sha256, never the body. `.env` / PEM download is allowed (wiki-warn).  
