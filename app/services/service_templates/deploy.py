...
def merge_secrets_into_env_files(
    ...
    for sk, sv in (secrets_map or {}).items():
        if sv is not None and str(sv) != "":
            env_map[sk] = str(sv)
    out[".env"] = (
        format_env_file(env_map, as_placeholders=False)
        if env_map
        else (out.get(".env") or "")
    )
    return out
...
def _project_path_for(server: Server, project_name: str) -> str:
    from .host_sync import project_path_for

    return project_path_for(server, project_name)
...
# Host sync lives in host_sync.py (keep import paths stable for callers/tests).
from .host_sync import (  # noqa: E402
